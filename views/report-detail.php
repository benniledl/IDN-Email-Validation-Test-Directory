<section class="app-card mb-4">
    <div class="app-card-body">
        ...
        <p class="mb-1"><strong>Risk:</strong> <span class="badge text-bg-dark text-uppercase"><?= htmlspecialchars((string)$report['severity_resolved'], ENT_QUOTES, 'UTF-8') ?></span></p>
        <p class="mb-1"><strong>Version tested:</strong> <?= htmlspecialchars((string)($report['wordpress_version'] ?: '-'), ENT_QUOTES, 'UTF-8') ?></p>
        <p class="mb-0"><strong>Comment:</strong> <?= nl2br(htmlspecialchars((string)($report['submission_comment'] ?: '—'), ENT_QUOTES, 'UTF-8')) ?></p>
...
<section class="app-card mb-4"><div class="app-card-body">
    <h2 class="h5">Test outcomes</h2>
    <div class="table-responsive"><table class="table app-table align-middle mb-0">
        <thead><tr><th>Email</th><th>Expected</th><th>Actual</th><th>Failure</th></tr></thead>
        <tbody><?php foreach ($tests as $test): ?><tr><td><code><?= htmlspecialchars((string)$test['email_address'], ENT_QUOTES, 'UTF-8') ?></code></td><td><?= (int)$test['expected_valid'] === 1 ? 'Valid' : 'Invalid' ?></td><td><?= htmlspecialchars((string)$test['actual_result'], ENT_QUOTES, 'UTF-8') ?></td><td><?= (int)$test['failure_detected'] === 1 ? 'Yes' : 'No' ?></td></tr><?php endforeach; ?></tbody>
    </table></div>
</div></section>

<section class="app-card"><div class="app-card-body">
    <div class="discussion-block">
        ...
        <form method="post" action="/reports/<?= (int)$report['id'] ?>/comments" class="row g-2 mb-3"><div class="col-md-4"><input class="form-control" name="author_name" placeholder="Your name"></div><div class="col-md-6"><input class="form-control" name="comment" placeholder="Write a comment"></div><div class="col-md-2 d-grid"><button class="btn btn-primary" type="submit">Post</button></div></form>
        <?php foreach ($comments as $comment): ?><article class="comment-item"><div class="small text-secondary"><?= htmlspecialchars((string)$comment['author_name'], ENT_QUOTES, 'UTF-8') ?> · <?= htmlspecialchars((string)$comment['created_at'], ENT_QUOTES, 'UTF-8') ?></div><p class="mb-0"><?= nl2br(htmlspecialchars((string)$comment['comment'], ENT_QUOTES, 'UTF-8')) ?></p><?php if (!empty($adminMode)): ?><form method="post" action="/reports/<?= (int)$report['id'] ?>/comments/<?= (int)$comment['id'] ?>/hide<?= !empty($adminToken) ? '?admin_token=' . urlencode((string)$adminToken) : '' ?>" class="mt-2"><input type="hidden" name="admin_token" value="<?= htmlspecialchars((string)$adminToken, ENT_QUOTES, 'UTF-8') ?>"><button class="btn btn-sm btn-outline-danger" type="submit">Hide comment</button></form><?php endif; ?></article><?php endforeach; ?>
    </div>
</div></section>
